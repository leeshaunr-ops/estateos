...
test('Real accounts, persistence, tenant scope, inspection/photo/PDF, vendor review, arrivals and billing',async()=>{
 ...
 const asset=await admin.req('assets',{propertyId:pa.id,name:'Generator',category:'Equipment',model:'Honda EU2200i',serial:'GEN-123',warranty:'2027-04',location:'Garage',mileage:'300',hours:'456'},201);
 ...
 const latestReport=await admin.raw(`properties/${pa.id}/assets/latest-inspections.pdf`);assert.equal(latestReport.status,200);assert.equal(latestReport.headers.get('content-type'),'application/pdf');const adminPdf=Buffer.from(await latestReport.arrayBuffer()).toString('latin1');assert.ok(adminPdf.includes('Honda EU2200i'));assert.ok(adminPdf.includes('GEN-123'));assert.ok(adminPdf.includes('456'),'report includes asset card details');
 assert.equal((await clientA.raw(`properties/${pb.id}/assets/latest-inspections.pdf`)).status,404,'clients cannot download another residence report');
 const employeeReport=await employee.raw(`properties/${pa.id}/assets/latest-inspections.pdf`);assert.equal(employeeReport.status,200);const employeePdf=Buffer.from(await employeeReport.arrayBuffer()).toString('latin1');assert.ok(employeePdf.includes('Residence A'));assert.ok(!employeePdf.includes('Private generator'),'employee reports only include assigned residences');assert.equal((await employee.raw(`properties/${pb.id}/assets/latest-inspections.pdf`)).status,404,'employees cannot download unassigned residence reports');
 const clientReport=await clientA.raw(`properties/${pa.id}/assets/latest-inspections.pdf`);assert.equal(clientReport.status,200,'clients can download their residence asset report');const clientPdf=Buffer.from(await clientReport.arrayBuffer()).toString('latin1');assert.ok(clientPdf.includes('Generator'));assert.ok(!clientPdf.includes('Private generator'),'clients only see assets at their own residence');
