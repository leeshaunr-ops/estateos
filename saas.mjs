...
export function createSaas({get,all,run,transaction,fail,text,note,id,hash,now,passwordHash,session,json,body,rate,audit,randomBytes,demos,demoSignup}){
 ...
 return async function handle(req,res,url,user){
  ...
  if(p==='/api/workspace-register'&&req.method==='POST'){
   ...
   await transaction(async()=>{
    ...
    if(demoSignup)await demoSignup.accepted(row.token_hash);
  ...
  if(p==='/api/workspace/settings'){
   ...
   const name=text(b.name,'Company name',160),support=b.supportEmail?email(b.supportEmail):'',logo=validateLogo(b.logoData,fail);
   if(b.primaryAdminId&&!await get("SELECT id FROM users WHERE id=? AND organization_id=? AND active=1 AND role='admin'",b.primaryAdminId,user.organization_id))fail(422,'Choose an active company administrator.');
   await transaction(async()=>{await run('UPDATE organizations SET name=? WHERE id=?',name,user.organization_id);await run('INSERT INTO workspace_settings(organization_id,support_email) VALUES(?,?) ON CONFLICT(organization_id) DO UPDATE SET support_email=excluded.support_email',user.organization_id,support);if(logo!==undefined)await run('UPDATE workspace_settings SET logo_data=? WHERE organization_id=?',logo,user.organization_id);if(b.primaryAdminId)await run('UPDATE workspace_settings SET primary_admin_id=? WHERE organization_id=?',b.primaryAdminId,user.organization_id);await audit(user,'workspace.settings_updated',user.organization_id);});json(res,201,{saved:true});return true;
  }
