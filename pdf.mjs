...
export function inspectionPdf(report,photos=[]){
 ...
 let logo=null;
 if(report.companyLogo){try{const bytes=Buffer.from(report.companyLogo.split(',')[1],'base64'),d=jpegSize(bytes);logo={...d,id:add(Buffer.concat([Buffer.from(`<< /Type /XObject /Subtype /Image /Width ${d.width} /Height ${d.height} /ColorSpace /${d.channels===1?'DeviceGray':'DeviceRGB'} /BitsPerComponent 8 /Filter /DCTDecode /Length ${bytes.length} >>\nstream\n`),bytes,Buffer.from('\nendstream')]))};}catch{}}
 function newPage(){page={stream:'',images:[]};pages.push(page);rect(0,0,612,74,C.brand);let x=40;if(logo){const scale=Math.min(100/logo.width,44/logo.height),w=logo.width*scale,h=logo.height*scale;rect(40,10,108,50,C.white);page.images.push(['Logo',logo.id]);page.stream+=`q ${w} 0 0 ${h} ${44+(100-w)/2} ${792-13-h} cm /Logo Do Q\n`;x=164;}const title=lines(report.company||'Residence care',572-x,17);title.slice(0,2).forEach((line,i)=>text(i===1&&title.length>2?line.slice(0,-3)+'...':line,x,8+i*21,17,C.white,true));text('PRIVATE RESIDENCE INSPECTION',x,59,8,[.91,.81,.64],true);rect(40,86,532,2,C.gold);y=104;}
