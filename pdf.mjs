...
export function inspectionPdf(report,photos=[]){
 ...
 function newPage(){page={stream:'',images:[]};pages.push(page);rect(0,0,612,74,[.97,.96,.93]);let x=40;if(logo){const scale=Math.min(100/logo.width,44/logo.height),w=logo.width*scale,h=logo.height*scale;page.images.push(['Logo',logo.id]);page.stream+=`q ${w} 0 0 ${h} ${44+(100-w)/2} ${792-13-h} cm /Logo Do Q\n`;x=164;}const title=lines(report.company||'Residence care',572-x,19);title.slice(0,2).forEach((line,i)=>text(i===1&&title.length>2?line.slice(0,-3)+'...':line,x,7+i*23,19,C.brand,true));text('PRIVATE RESIDENCE INSPECTION',x,59,8,C.muted,true);rect(40,86,532,2,C.gold);y=104;}
