...
export function inspectionPdf(report,photos=[]){
 ...
 let logo=null;
 if(report.companyLogo){try{const bytes=Buffer.from(report.companyLogo.split(',')[1],'base64');if(report.companyLogo.startsWith('data:image/png;')){const d=decodeLogoPng(bytes),alpha=deflateSync(d.alpha),rgb=deflateSync(d.rgb);const mask=add(Buffer.concat([Buffer.from(`<< /Type /XObject /Subtype /Image /Width ${d.width} /Height ${d.height} /ColorSpace /DeviceGray /BitsPerComponent 8 /Filter /FlateDecode /Length ${alpha.length} >>\nstream\n`),alpha,Buffer.from('\nendstream')]));logo={...d,id:add(Buffer.concat([Buffer.from(`<< /Type /XObject /Subtype /Image /Width ${d.width} /Height ${d.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /SMask ${mask} 0 R /Filter /FlateDecode /Length ${rgb.length} >>\nstream\n`),rgb,Buffer.from('\nendstream')]))};}else{const d=jpegSize(bytes);logo={...d,id:add(Buffer.concat([Buffer.from(`<< /Type /XObject /Subtype /Image /Width ${d.width} /Height ${d.height} /ColorSpace /${d.channels===1?'DeviceGray':'DeviceRGB'} /BitsPerComponent 8 /Filter /DCTDecode /Length ${bytes.length} >>\nstream\n`),bytes,Buffer.from('\nendstream')]))};}}catch{}}
 function newPage(){page={stream:'',images:[]};pages.push(page);let x=logo?164:40;const title=lines(report.company||'Residence care',572-x,19),labelTop=32+(Math.min(title.length,2)-1)*23,headerBottom=Math.max(labelTop+20,logo?64:0);rect(0,0,612,headerBottom,[.97,.96,.93]);if(logo){const scale=Math.min(100/logo.width,44/logo.height),w=logo.width*scale,h=logo.height*scale;page.images.push(['Logo',logo.id]);page.stream+=`q ${w} 0 0 ${h} ${44+(100-w)/2} ${792-13-h} cm /Logo Do Q\n`;x=164;}title.slice(0,2).forEach((line,i)=>text(i===1&&title.length>2?line.slice(0,-3)+'...':line,x,7+i*23,19,C.brand,true));text('PRIVATE RESIDENCE INSPECTION',x,labelTop,8,C.muted,true);rect(40,headerBottom+12,532,2,C.gold);y=headerBottom+30;}
