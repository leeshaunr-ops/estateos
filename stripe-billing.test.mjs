...
for(const engine of ['sqlite','postgres'])test('company checkout, paid activation, retry and capacity: '+engine,async()=>{
 ...
 try{
  ...
  paid=true;live=false;await assert.rejects(billing.reconcile('a'),/test checkout/);live=true;await billing.reconcile('a');assert.equal((await billing.state('a')).quote.storageGB,10);assert.equal((await billing.state('a')).status,'active');
