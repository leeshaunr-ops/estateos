...
async function api(req, res, url, user) {
    ...
    if (p === '/api/clients') {
        ...
        (await transaction(async () => { (await run('INSERT INTO arrivals(id,property_id,created_by,arrival_at,needs,status,items,created_at,version,room_status) VALUES(?,?,?,?,?,?,?,?,?,?)', key, pRow.id, user.id, b.arrivalAt, note(b.needs), 'submitted', JSON.stringify(items), now(), 1, JSON.stringify(roomStatus))); (await notifyProperty(pRow, 'Arrival preparation requested', key)); (await audit(user, 'arrival.created', key)); }));
        result = { id: key };
    }
